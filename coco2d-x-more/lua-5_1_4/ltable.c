...
static Node *hashnum (const Table *t, lua_Number n) {
  ...
  if (luai_numeq(n, 0))  /* avoid problems with -0 */
    return gnode(t, 0);
  memcpy(a, &n, sizeof(a));
  for (i = 1; i < numints; i++) a[0] += a[i]; //信息压缩？
...
static Node *mainposition (const Table *t, const TValue *key) {
  switch (ttype(key)) {
    case LUA_TNUMBER:
      return hashnum(t, nvalue(key)); //基础数值,没有固定地址, 做特别处理
    case LUA_TSTRING:
      return hashstr(t, rawtsvalue(key)); //会使用 string 在全局字符串表里的 hash 方法取值，结果值再对桶大小取余，确定位置
    case LUA_TBOOLEAN:
      return hashboolean(t, bvalue(key)); //基础数值, 会使用 value->b (int b) 的值对桶大小取余，确定位置
    case LUA_TLIGHTUSERDATA:
      return hashpointer(t, pvalue(key)); //会使用 value->p 的地址值对桶大小取余，确定位置
    default:
      return hashpointer(t, gcvalue(key)); //会使用 value->gc 的地址值对桶大小取余，确定位置
  }
}
...
static TValue *newkey (lua_State *L, Table *t, const TValue *key) {
  ...
  if (!ttisnil(gval(mp)) || mp == dummynode) {
    ...
    othern = mainposition(t, key2tval(mp));//.faq
